    ...
    return (
      <>
        ...
        <form onSubmit={async (e) => {
          ...
          const correct = e.target.correct_answer.value;
          const incorrect = e.target.incorrect_answers.value.split(',').filter(Boolean);
          const response = await fetch('/api_quiz', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
            body: JSON.stringify({ question, correct, incorrect })
          });
          if (response.ok) {
            setQuestions([...questions, { question, correct, incorrect }]);
            e.target.reset();
          } else {
